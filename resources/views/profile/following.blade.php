        ...
        <div class="row justify-content-center">
            <div class="col-12">
                <div class="user-profile-card">
                    <div class="row">
                        @include('profile.partials.profile_card', ["tab" => "following"])
                    </div>
                </div>
                <div class="profile-data profile-following mt-4">
                                    ...
                                    <div class="profile-card profile-card--following">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <div class="profile-image-wrapper">
                                                <a class="text-decoration-none" href="{{ route('profile.index', ['user' => $followingUser->username]) }}">
                                                    @isset($followingUser->profileImage)
                                                        <img class="img-fluid rounded-circle" src="{{ asset('storage/' . $followingUser->profileImage->path) }}" alt="{{ $followingUser->first_name }} {{ $followingUser->last_name }}">
                                                    @else
                                                        <img width="200" height="200" class="border img-fluid rounded-circle" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQYV2P48fPXfwAJwwPruND9lQAAAABJRU5ErkJggg==" alt="{{ $followingUser->first_name }} {{ $followingUser->last_name }}">
                                                    @endisset
                                                </a>
                                            </div>
                                            <div class="profile-card--meta w-100 ml-3">
                                                <h4 class="profile-card--title m-0 text-truncate"><a class="text-decoration-none" href="{{ route('profile.index', ['user' => $followingUser->username]) }}">{{ $followingUser->first_name }} {{ $followingUser->last_name }}</a></h4>
                                                <div class="profile-card--username mb-2 text-muted text-truncate">{{ '@' . $followingUser->username }}</div>
                                                @if(Auth::check() && Auth::User()->id !== $followingUser->id)
                                                    <a href="{{ route('unfollow.user', ['user' =>  $followingUser->username ]) }}" class="btn btn-light btn-sm border unfollow-user">Unfollow</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                    ...
                    @else
                        <div class="empty-data text-center">
                            ...
                            @endif
                            <h5 class="mt-4 font-weight-normal">{{$user->first_name}} {{$user->last_name}} is not following anyone</h5>
                            @if(Auth::check() && Auth::User()->id === $user->id)
                                <p class="text-muted">Follow other people to get know about their products first when the publish them</p>
                            @endif
                        </div>
                    @endif
