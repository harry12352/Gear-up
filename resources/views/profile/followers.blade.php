        ...
        <div class="row justify-content-center">
            <div class="col-12">
                <div class="user-profile-card">
                    <div class="row">
                        @include('profile.partials.profile_card', ["tab" => "followers"])
                    </div>
                </div>
                <div class="profile-data profile-followers mt-4">

                    @if($user->followers && count($user->followers) > 0)
                        <div class="row">
                            @foreach($user->followers as $follower)
                                @php
                                    $followersUser = $follower->followerUser
                                @endphp
                                <div class="col-lg-3 col-sm-4 col-6">
                                    <div class="profile-card profile-card--followers">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <div class="profile-image-wrapper">
                                                <a class="text-decoration-none" href="{{ route('profile.index', ['user' => $followersUser->username]) }}">
                                                    @isset($followersUser->profileImage)
                                                        <img class="img-fluid rounded-circle" src="{{ asset('storage/' . $followersUser->profileImage->path) }}" alt="{{ $followersUser->first_name }} {{ $followersUser->last_name }}">
                                                    @else
                                                        <img width="200" height="200" class="img-fluid rounded-circle" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQYV2P48fPXfwAJwwPruND9lQAAAABJRU5ErkJggg==" alt="{{ $followersUser->first_name }} {{ $followersUser->last_name }}">
                                                    @endisset
                                                </a>
                                            </div>
                                            <div class="profile-card--meta w-100 ml-3">
                                                <h4 class="profile-card--title m-0 text-truncate"><a class="text-decoration-none" href="{{ route('profile.index', ['user' => $followersUser->username]) }}">{{ $followersUser->first_name }} {{ $followersUser->last_name }}</a></h4>
                                                <div class="profile-card--username mb-2 text-muted text-truncate">{{ '@' . $followersUser->username }}</div>
                                                @if(Auth::check() && Auth::User()->id !== $followersUser->id)
                                                    @if(isUserFollowing($followersUser))
                                                        <a href="{{ route('unfollow.user', ['user' =>  $followersUser->username ]) }}" class="btn btn-light btn-sm border unfollow-user">Unfollow</a>
                                                    @else
                                                        <a href="{{ route('follow.user', ['user' =>  $followersUser->username ]) }}" class="btn btn-light btn-sm border follow-user">Follow</a>
                                                    @endif
                                                @endif
                                            </div>
                                        </div>
                    ...
                    @else
                        ...
                        <div class="empty-data text-center">
                            ...
                            @if(Auth::check() && Auth::User()->id === $user->id)
                                <svg id="f20e0c25-d928-42cc-98d1-13cc230663ea" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="820.16" height="780.81" viewBox="0 0 820.16 780.81">
                                    <defs>
                                    ...
                                    <rect x="375.66" y="136.55" width="199.84" height="47.27" transform="translate(-212.94 43.72) rotate(-12.11)" fill="#028f69"/>
                                    <path d="M460.93,91.9a27.93,27.93,0,1,0,33.17,21.45A27.93,27.93,0,0,0,460.93,91.9ZM470.17,135a16.12,16.12,0,1,1,12.38-19.14A16.12,16.12,0,0,1,470.17,135Z" transform="translate(-189.92 -59.59)" fill="#028f69"/>
                                    ...
                                    <path d="M666.86,118a27.93,27.93,0,1,0,27.93,27.93A27.93,27.93,0,0,0,666.86,118Zm0,44.05A16.12,16.12,0,1,1,683,145.89,16.12,16.12,0,0,1,666.86,162Z" transform="translate(-189.92 -59.59)" fill="#028f69"/>
                                    <g opacity="0.5">
                            ...
                            @endif
                            <h5 class="mt-4 font-weight-normal">No followers found</h5>
                            @if(Auth::check() && Auth::User()->id === $user->id)
                                <p class="text-muted">Oops... we don't see anyone following you.<br>Publish more products and engage with other people to get more followers</p>
                            @endif
                        </div>
                    @endif
